    ...
    async handleFetchCurrencyData() {
      try {
        const req = await fetch(
          `https://marketdata.tradermade.com/api/v1/live_currencies_list?api_key=bcPlgfur113zCz71ZrMm`,
        );
        ...
        <div class="inputs_container">
          <div class="dropdown" @click="toggleDropdown('one')">
            <div class="selected-option">
              <span
                :class="`flags currency-flag currency-flag-${flagOne.toLowerCase()}`"
              ></span>
              {{ flagOne }}
              <span class="dropdown-icon" :class="{ open: isDropdownOpenOne }"
                >&#9660;</span
              >
            </div>
            <div v-if="isDropdownOpenOne" class="options-list">
              ...
              >
                <span
                  :class="`currency-flag currency-flag-${currency.toLowerCase()}`"
                ></span>
                {{ currency }}
              </div>
            </div>
          ...
          <div class="dropdown" @click="toggleDropdown('two')">
            <div class="selected-option">
              <span
                :class="`flags currency-flag currency-flag-${flagTwo.toLowerCase()}`"
              ></span>
              {{ flagTwo }}
              <span class="dropdown-icon" :class="{ open: isDropdownOpenTwo }"
                >&#9660;</span
              >
            </div>
